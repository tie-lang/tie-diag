...
$nameMap = @{
    '字符串未闭合' = @('字符串字面量缺少结束引号（词法扫描）', '补上结束引号；检查多行字符串是否需要拼接')
    '字符字面量未闭合' = @('字符字面量缺少结束引号', '补上结束引号')
    '字符字面量只能包含一个字符' = @('字符字面量里放了多个字符/码元', '只保留一个字符；转义序列用 \\u 或直接字符')
    '无法识别的字符' = @('扫描到非法符号', '删除或替换为合法符号')
    '未知转义序列' = @('\\ 后的转义符不是支持项', '使用支持的转义（\\n \\t \\r \\" \\\\ 等）')
    '块注释未闭合' = @('块注释 /* ... */ 没有闭合 */', '补上 */，或把误写的 /* 改成 /')
    '转义符后缺少字符' = @('转义符 \\ 后没有字符（字符串尾）', '补上转义字符，或检查字符串是否提前结束')
    '期望标识符' = @('该位置期望标识符（名字/关键字），实际是别的符号', '检查拼写与关键字位置；是否漏了名字')
    '期望类型' = @('该位置期望类型表达式', '补上类型标注，检查类型名/泛型实参')
...
$prefMap = @(
    ...
    @{ P = '赋值'; C = '赋值目标未声明/类型不匹配/const 赋值'; F = '先声明；类型一致；const 不可改' }
